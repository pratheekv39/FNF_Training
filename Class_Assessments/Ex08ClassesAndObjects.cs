...
namespace SampleConApp
{
    ...
    internal class Ex08ClassesAndObjects
    {
        ...
        {
            int empId = ConsoleUtil.GetInputInt("Enter Employee ID to delete");
        ...
        {
            int empId = ConsoleUtil.GetInputInt("Enter Employee ID to update");
            Employee existingEmp = employeeRepo.GetEmployee(empId);
            if (existingEmp == null)
            {
                Console.WriteLine($"No Employee found with ID {empId} to update.");
                return;
            }
            ...
            updatedEmp.EmpName = ConsoleUtil.GetInputString("Enter Employee Name");
            updatedEmp.EmpAddress = ConsoleUtil.GetInputString("Enter Employee Address");
            updatedEmp.EmpSalary = ConsoleUtil.GetInputDouble("Enter Employee Salary");
            updatedEmp.Designation = ConsoleUtil.GetInputString("Enter Employee Designation");

            employeeRepo.UpdateEmployee(updatedEmp);
        }
